...
class AMPTransformerMultiTaskAdaptBuilder(AMPBuilder):
    ...
    class Network(AMPTransformerMultiTaskBuilder.UnifiedNetworkClass):
        ...
        def __init__(self, params, **kwargs):
            ...
            if self.has_extra:
                self.extra_task_obs_size = self.task_obs_each_size[self.each_subtask_name.index("new_extra")]
                self.extra_id = self.each_subtask_name.index("new_extra")
            ...
            if self.has_extra:
                ...
                self.task_encoder[self.extra_id] = self._build_mlp(**mlp_args)

                for m in self.task_encoder[self.extra_id].modules(): # zero init
        ...
        def _eval_Transformer(self, obs, not_normalized_obs):
            ...
            if self.has_extra:
                extra_task_obs = obs[..., self.self_obs_size:][..., self.task_obs_each_indx[self.extra_id]:self.task_obs_each_indx[self.extra_id + 1]]
                extra_task_token = self.task_encoder[self.extra_id](extra_task_obs).unsqueeze(1) # the only place where the obs is used
